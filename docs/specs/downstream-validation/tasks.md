...
| W2.2 | ✅ done 2026-05-20 — static audit + CI perf-gate cross-check at [`w2-perf-audit.md`](./w2-perf-audit.md). PR #72 advisory flagged a single-axis regression on `rag_pipeline_run.cpu/.wall` (~+34 % over threshold); other axes ok. Static diff finds no plausible mechanism for the magnitude — working hypothesis is sub-millisecond benchmark noise. **Next:** re-run perf workflow on main; eliminate structlog hypothesis if regression persists; widen N=30→50 to tighten the gate before W3.1 promotion. Four Phase-5 micro-opts surfaced; reranker is LLM-bound (no findings). `attune-ai:performance_audit` MCP tool itself is broken (separate bug). | attune-rag | If audit finds optimizations, file as Phase 5 work; no Adding during freeze. |
