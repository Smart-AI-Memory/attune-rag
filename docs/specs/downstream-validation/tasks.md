...
| W3.2 | ✅ done 2026-05-20 — `.github/workflows/downstream-attune-gui.yml` test step had `continue-on-error: true` removed, so an attune-gui editor + RAG test failure now fails the workflow and blocks merge. PR-comment step still runs under `if: always()` so reviewers see the verdict regardless. Shipped as [PR #81](https://github.com/Smart-AI-Memory/attune-rag/pull/81) per design.md §4's promotion ramp + Decision 2 of the v1.0 roadmap (attune-gui is the gating downstream for Phase 4). Rollback: re-add `continue-on-error: true` to the test step. | attune-rag | |
| W3.3 | ✅ done 2026-05-22 — `pytest --cov` baseline 88.53 % aggregate; raised to **90.04 %** by adding 20 tests across `test_editor_references.py` (+8) and `test_editor_rename.py` (+12) for branches end-to-end fixtures didn't exercise (duck-typed corpus shapes, empty content/path entries, no-frontmatter docs, block-style scan terminators, unsupported rename kinds, normalizer rejections, `FileMove.to_dict`, apply-rename failure modes). `editor/references.py` 88 → 99 %; `editor/rename.py` 87 → 92 %. Also `pragma: no cover` on two Protocol-stub ellipses in `corpus/help_adapter.py`. All public `__all__` modules now ≥ 90 %. 849/849 tests green; zero `### Added`. | attune-rag | Coverage targets ~90% on `__all__` symbols, no Added. |
