...
| W2.2 | ✅ done 2026-05-20 — static audit + CI perf-gate cross-check at [`w2-perf-audit.md`](./w2-perf-audit.md). PR #72's advisory flagged `rag_pipeline_run.cpu/.wall` over threshold (+33.9/+34.0 %); PR #74's second reading on identical code put it back under (+10.8/+11.0 %) — **single-point noise, not regression**. **Blocker before W3.1 promotion:** widen N=30→50 for `rag_pipeline_run` to close the ±23 % single-run swing. Reranker is LLM-bound (no findings); four Phase-5 micro-opts surfaced; `attune-ai:performance_audit` MCP tool itself is broken (separate bug). | attune-rag | If audit finds optimizations, file as Phase 5 work; no Adding during freeze. |
