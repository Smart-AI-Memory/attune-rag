...
| M2.3 | Run paired benchmark off+on against current HEAD, n = 40 (full golden), with `--with-faithfulness --compare-thinking --json`. Output: `artifacts/calibration/thinking-2026-05-Q.json`. | attune-rag | not started | One-shot CLI. ~30 min, ~120 LLM calls. Set Q = the date of the run. |
| M2.4 | Generate labeling kit at n = 30 (15 shift + 15 random + 0 controls; or 13 shift + 14 random + 3 controls — choose in design.md before running). Output: `ground-truth-2026-05-Q.template.md`. | attune-rag | not started | Use `--seed 42` for reproducibility. |
...
- [ ] M2 — kit extension + paired benchmark + template
