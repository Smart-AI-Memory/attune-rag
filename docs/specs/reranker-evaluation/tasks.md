...
| # | Task | Layer | Notes |
|---|---|---|---|
| M1.1 | Create `scripts/measure_reranker.py` skeleton: `argparse` CLI mirroring `scripts/measure_baseline_variance.py`. Flags: `--baseline-queries`, `--paraphrased-queries`, `--rerank-runs N` (default 5), `--out PATH` (report path), `--candidate-multiplier` (default 3). | attune-rag | Pure stdlib + existing `attune_rag` imports. No new deps. Use the shared `attune_rag._scoring.score_queries` helper for P@1 / R@3 — extracted in the `scripts/measure_corpus.py` PR; do **not** duplicate the loop. |
...
| M1.3 | Implement Run A (`rerank=off`): iterate baseline + paraphrased query YAMLs, capture per-query top-3 paths, compute P@1 and R@3 against ground truth via `score_queries`. **Strict-dominance check (R1):** assert numbers reproduce the bundled baseline. **Note (2026-05-22):** the spec originally cited 1.00 / 1.00 / 0.9125 / 1.00 — those are the rerank-*on* aspirational numbers, not rerank-*off*. Today's actual rerank-off numbers (verified by `tests/golden/measure_corpus_bundled.golden.md`) are 1.00 / 1.00 / 0.8750 / 0.9875. Update Run A's assertion to these. | attune-rag | This is the regression net — if Run A doesn't reproduce, the diagnostic is wrong. Cross-check with `scripts/measure_corpus.py --corpus-bundled --queries tests/golden/queries.yaml --paraphrased tests/golden/queries_paraphrased.yaml --frozen-timestamp <iso> --output /tmp/check.md` should byte-equal the golden file. |
