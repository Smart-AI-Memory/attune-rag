...
> Decisions locked in [decisions.md](decisions.md). Sections below are
> corrected where the scoping audit found them stale; corrections are
> marked inline rather than silently applied.
...
|---|---|---|
| Public surface | Documented, snapshot-tested, frozen. | **Same surface.** No additions — except the D7 ruling, if Option A. |
...
> one open item is the `DirectoryCorpus` override — that is D7, the
> single scope question awaiting Patrick's ruling. The telemetry
> config-surface reservation and the standard cut work remain as
> written.
...
**Caveat if D7 lands as Option A.** Adding the `DirectoryCorpus`
override *does* move the surface, which breaks the identity above. In
that case M0.5 lands first, the snapshot updates, and consumer
validation runs against a 0.9.x that already carries the new symbol —
or M0 re-validates after it. Sequencing M0.5 before M0.1–M0.3 keeps
one ordering instead of two.
