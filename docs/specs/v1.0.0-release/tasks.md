...
| M4.2 | If any `1.0.z` hotfix release fires inside the seven-day window, **restart the seven-day clock** at the hotfix publish time (per [ROADMAP-v1.md](../ROADMAP-v1.md) Phase 5 gate). | attune-rag | A hotfix is also a signal to re-examine whether the cut was premature; log the cause in the Phase-5 retrospective. |
