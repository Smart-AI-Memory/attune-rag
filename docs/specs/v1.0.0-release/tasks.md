...
| M3.6 | **Remove the FIVE shim modules** `_autocomplete`, `_lint`, `_references`, `_rename`, `_schema` (ruled 2026-08-10) + their shim test `tests/unit/test_editor_rename_shims.py`. **Do NOT remove `_regex.py`** — live internal module (shared patterns imported by `rename`/`references`), not a shim. Zero-impact verified at M1.4 (shims not in `__all__`, zero consumer imports). | attune-rag | CHANGELOG `### Removed` entry + POLICY §3 note updated in M2.2. Count error ("six") caught 2026-08-10 before it could delete a live module. |
