...
| M3.6 | **Remove the six `editor/_*.py` shims** (ruled 2026-08-10) + their shim test `tests/unit/test_editor_rename_shims.py`. Zero-impact verified at M1.4 (not in `__all__`, zero consumer imports). Legal under 0.x policy since 0.3.0; the major bump is the natural landing. | attune-rag | CHANGELOG `### Removed` entry + POLICY §3 note updated in M2.2. |
