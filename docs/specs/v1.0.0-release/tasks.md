...
| M1.4 | ✅ **answered 2026-08-10 — the shims were NEVER removed.** The FIVE shim modules (`_autocomplete`, `_lint`, `_references`, `_rename`, `_schema`) present at 0.9.0, all `DeprecationWarning`-marked since 0.2.0 (`_regex.py` also exists but is a LIVE internal module — shared compiled patterns imported by `rename`/`references` — NOT a shim; count corrected 2026-08-10 after the M1 write-up briefly said six), while POLICY.md still promises "removed in 0.3.0" — seven minors late. | attune-rag | **Removal is zero-impact and verified safe:** not in the public `__all__` snapshot, ZERO consumer imports (attune-ai src, gui sidecar, help src all grep-clean), only self-references + their own shim test. **Recommendation: remove in the cut PR (M3)** — a major bump is exactly where removals belong, and it is the last exit before 2.0.0. **RULED 2026-08-10 (Patrick): remove at the cut.** M3 gains task M3.6 (shim removal in the cut PR); M2.2 fixes POLICY.md's 0.3.0 claim; CHANGELOG [1.0.0] gains `### Removed`. |
...
| M1.2 | ✅ **done 2026-08-10** — delta-scoped review: read every public-surface module changed since W2.1 (2026-05-20): `retrieval.py` (#194 preview-strip + ALIASES_WEIGHT), `expander.py` (tier resolution), `providers/claude.py` (fable routing, cache TTL, refusal translation), `corpus/directory.py` (full read during D7). **No blocking findings.** | attune-rag | Two observations to 1.0.x backlog: (1) `_ALIASES_BLOCK_RE.sub(count=1)` can strip a body-level `aliases:` line in a template with no frontmatter aliases — preview-only, sub-scoring impact; (2) `KeywordRetriever.__init__` shadows the documented class constant `MIN_SCORE` via instance attribute — stylistic. **Second pass completed 2026-08-10** (day 0 of the watch, ruled at the retro) — see [review-pass-2.md](review-pass-2.md): rename.py apply-path rollback verified line-by-line, pipeline.py whole-module, risk-pattern sweep over the rest. No blocking defects; 3 observations to 1.0.x. Cadence satisfied. |
...
| M2.1 | ✅ **done 2026-08-10** — POLICY **§8 Support window**: 6 months past the next minor (D5), scope-honesty paragraph verbatim with the consumer corrected to attune-ai, revisit trigger included. | attune-rag | Landed in the M2 docs-roll PR. |
| M2.2 | ✅ **done 2026-08-10** — POLICY **§9 Deprecation under 1.x** (D6 text). Plus the M1-mandated fiction sweep: preamble present-tense, §2 "(future)" dropped, §3+§5 "removed in 0.3.0" → removed-at-1.0.0 (M3.6), §4.1 "alpha" → Beta. | attune-rag | §3 stays for historical context, as specced. |
| M2.3 | ✅ **done 2026-08-10** — headline had NO maturity claim (the predicted no-op held; nothing invented). Real fixes found instead: Status section was four minors stale at "0.5.1" → refreshed to the 0.9.x line; shim text corrected; §8–§9 cross-link added; ecosystem footer notes author+gui archived. | attune-rag | Public-API section unchanged. |
| M2.4 | ✅ **done 2026-08-10** — `[1.0.0]` staged as UNRELEASED: `### Added` (support window, Beta→Stable classifier), `### Changed` (deprecation tightens), **`### Removed` (the five shims — added per the M1.4 ruling; `_regex.py` explicitly excluded)**. | attune-rag | Date + UNRELEASED marker resolve at M3.3 tag time. |
| **M3** | **Cut & release** — must run as one PR + tag pair. | | |
| M3.1 | ✅ **done (cut PR)** — classifier flipped `4 - Beta` → `5 - Production/Stable`. | attune-rag | **Corrected (D2)** — the package has never been `3 - Alpha`; it has sat at Beta since the initial scaffold. One-line edit; the "Beta is intentionally skipped" rationale is void and deleted. |
| M3.2 | ✅ **done (cut PR)** — `0.9.0` → `1.0.0` in both files + uv.lock. | attune-rag | **Corrected (D1)** — parent is 0.9.x, not 0.2.x. Verified by the `attune-release-check` skill before tag. |
| M3.6 | ✅ **done (cut PR)** — five shims + shim test removed; `_regex.py` untouched (live internal module). Full suite green post-removal (1091 passed). | attune-rag | CHANGELOG `### Removed` entry + POLICY §3 note updated in M2.2. Count error ("six") caught 2026-08-10 before it could delete a live module. |
| M3.5 | ✅ **done (cut PR)** — Q4 `__all__` alphabetised (snapshot is a frozenset — no EXPECTED change needed); Q3 local rename `validator` → `keyword` (**note: items.md's `err.keyword` does not exist in jsonschema** — the attribute stays `err.validator`; only the local name changed). | attune-rag | D9. Ordering-only and local-variable-only respectively — no symbols added or removed. The cut is the moment this is cheapest; after it, `__all__` churn costs a deprecation cycle. |
| M3.3 | ✅ **done 2026-08-10** — `attune-release-check` pre-flight clean; tagged `v1.0.0` from merge SHA `1351d79` (full-SHA `--target`); publish run [31358524673](https://github.com/Smart-AI-Memory/attune-rag/actions/runs/31358524673) approved by Patrick at the `pypi` gate; **1.0.0 verified live on PyPI** (wheel + sdist, classifier `Production/Stable`). | attune-rag | |
| M3.4 | ✅ **done 2026-08-10** — [release notes](https://github.com/Smart-AI-Memory/attune-rag/releases/tag/v1.0.0): burn-in evidence table, POLICY §8/§9 links with the scope-honesty pointer, shim-removal callout, drop-in-from-0.9.x statement. | attune-rag | |
| **M4** | **Post-release watch** — passive monitoring, 7+ days. | | |
| M4.1 | ✅ **retired early 2026-08-10** — the chair pulled 1.1.0 forward the same day, superseding the 1.0.z hotfix channel. Zero hotfixes shipped or pending at retirement. See [exit-summary.md](exit-summary.md). | attune-rag | Daily-briefing check-in obligation retired with it. |
...
| M4.3 | ✅ **done 2026-08-10** — Phase 5 closed via [exit-summary.md](exit-summary.md) (early close, chair-directed; window superseded-not-met is recorded honestly). No `post-1.0.0-watch/` spec: M1.3 had zero gap items and the M1.2/pass-2 observations are non-blocking ordinary backlog. | attune-rag | |
