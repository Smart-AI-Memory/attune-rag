...
# A section opening with ``> **Freeze override`` in its header
# blockquote ratifies an authorized Added entry per
# ``docs/specs/downstream-validation/cadence-week-schema.md``. Any
# Added bullets in that section don't reset the freeze clock.
_OVERRIDE_RE = re.compile(r"^>\s*\*\*Freeze override", re.MULTILINE | re.IGNORECASE)
...
class ChangelogSection:
    ...
    maps category name → bullet count. ``is_override`` is True when
    the section header carries a ``> **Freeze override`` blockquote,
    indicating any Added bullets are authorized exceptions.
    """
...
def parse_changelog(text: str) -> list[ChangelogSection]:
    ...
    for i, (pos, version, heading_date) in enumerate(headings):
        ...
        is_override = _section_has_override_header(block)
        sections.append(
            ChangelogSection(
                version=version,
                release_date=heading_date,
...
def _section_has_override_header(block: str) -> bool:
    """Detect a ``> **Freeze override`` blockquote in a section's header.

    Scans from the ``## [...]`` line up to the first ``### Category``
    sub-heading. The override flag is a section-level attribute set by
    the PR that authored the addition; later sections inherit nothing.
    """
    m = _CATEGORY_RE.search(block)
    header = block[: m.start()] if m else block
    return bool(_OVERRIDE_RE.search(header))
...
def aggregate_counts(sections: list[ChangelogSection]) -> dict[str, int]:
    ...
    The returned dict carries one extra synthetic key,
    ``"Added_override"``, holding the subset of the ``Added`` count
    that came from override-flagged sections. ``render_report`` uses
    this to compute effective Added for the ON TRACK / RESET status
    without losing the raw count in the report table.
    """
    out: dict[str, int] = dict.fromkeys(CATEGORIES, 0)
    out["Added_override"] = 0
    for section in sections:
        for category, n in section.categories.items():
            if category in out:
                out[category] += n
        if section.is_override:
            out["Added_override"] += section.categories.get("Added", 0)
    return out
...
def render_report(
    *,
    ...
    """
    added_raw = counts.get("Added", 0)
    added_override = counts.get("Added_override", 0)
    effective_added = added_raw - added_override
    status_core = "ON TRACK" if effective_added <= 0 else "RESET"
    ...
    lines.append(
        "> Effective `Added` (raw minus override-flagged) resets the "
        "Phase 4 freeze clock; status is derived from that count."
    )
    lines.append("")
    ...
    for cat in CATEGORIES:
        n = counts.get(cat, 0)
        suffix = ""
        if cat == "Added" and added_override > 0:
            suffix = f"  ({added_override} under override)"
        elif cat == "Changed" and n > 0 and releases:
